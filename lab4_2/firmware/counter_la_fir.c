...
//* initialize fir *//
	reg_mprj_datal = 0xAB410000;
	config_set(data_length) ;
	tap_write();
	reg_mprj_datal = 0xAB420000;
	int a = 0;
//*   run fir	   *// 
	if(reg_fir_control == FIR_IDLE) reg_fir_control = FIR_START; 
	else reg_mprj_datal = 0xABFF0000;
	reg_mprj_datal = 0xAB430000;
	for(int i=0 ; i < data_length ; i++){
		if((reg_fir_control == Y_READY)|| (reg_fir_control == XY_READY)){
			reg_mprj_datal = reg_fir_y >> 1;
			a = a+1;
		}	
		while ((reg_fir_control != X_READY) && (reg_fir_control != XY_READY));
			reg_fir_x = data[i];
	}
	while(a < data_length){
		if((reg_fir_control == Y_READY)|| (reg_fir_control == XY_READY)){
